module falcon_intt32_dma_accel (
    input logic clk_i,
    input logic rst_ni,
  ...
  localparam logic [31:0] NI32 = 32'd2048;  // R / 32 mod q, R = 4091
  ...
  task automatic scale_final(inout logic [31:0] x [0:N-1]);
    begin
      for (int i = 0; i < N; i++) begin
        x[i] = montgomery_mul(x[i], NI32);
      end
    end
  endtask

  task automatic intt32_compute(inout logic [31:0] x [0:N-1]);
    int unsigned m;
    int unsigned hm;
    int unsigned t;
    int unsigned i;
