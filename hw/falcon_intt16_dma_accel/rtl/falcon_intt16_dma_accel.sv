module falcon_intt16_dma_accel (
    input logic clk_i,
    input logic rst_ni,
  ...
  localparam logic [31:0] NI16 = 32'd4096;  // R / 16 mod q, R = 4091
  ...
  task automatic scale_final(inout logic [31:0] x [0:N-1]);
    begin
      for (int i = 0; i < N; i++) begin
        x[i] = montgomery_mul(x[i], NI16);
      end
    end
  endtask

  task automatic intt16_compute(inout logic [31:0] x [0:N-1]);
    begin
      /*
       * Mini-iNTT16 siguiendo mq_iNTT.
       *
       * iGMb[1..15]:
      ...
      // m = 16, hm = 8, t = 1, s = iGMb[8..15]
      intt_butterfly(x[0],  x[1],  32'd2579);
      intt_butterfly(x[2],  x[3],  32'd4751);
      intt_butterfly(x[4],  x[5],  32'd6464);
      intt_butterfly(x[6],  x[7],  32'd11703);
      intt_butterfly(x[8],  x[9],  32'd7023);
      intt_butterfly(x[10], x[11], 32'd2812);
      intt_butterfly(x[12], x[13], 32'd5890);
      intt_butterfly(x[14], x[15], 32'd10698);

      // m = 8, hm = 4, t = 2, s = iGMb[4..7]
      intt_butterfly(x[0],  x[2],  32'd2530);
      intt_butterfly(x[1],  x[3],  32'd2530);

      intt_butterfly(x[4],  x[6],  32'd6014);
      intt_butterfly(x[5],  x[7],  32'd6014);

      intt_butterfly(x[8],  x[10], 32'd7947);
      intt_butterfly(x[9],  x[11], 32'd7947);

      intt_butterfly(x[12], x[14], 32'd5329);
      intt_butterfly(x[13], x[15], 32'd5329);

      // m = 4, hm = 2, t = 4, s = iGMb[2..3]
      intt_butterfly(x[0],  x[4],  32'd1081);
      intt_butterfly(x[1],  x[5],  32'd1081);
      intt_butterfly(x[2],  x[6],  32'd1081);
      intt_butterfly(x[3],  x[7],  32'd1081);

      intt_butterfly(x[8],  x[12], 32'd1229);
      intt_butterfly(x[9],  x[13], 32'd1229);
      intt_butterfly(x[10], x[14], 32'd1229);
      intt_butterfly(x[11], x[15], 32'd1229);

      // m = 2, hm = 1, t = 8, s = iGMb[1]
      intt_butterfly(x[0], x[8],  32'd4401);
      intt_butterfly(x[1], x[9],  32'd4401);
      intt_butterfly(x[2], x[10], 32'd4401);
      intt_butterfly(x[3], x[11], 32'd4401);
      intt_butterfly(x[4], x[12], 32'd4401);
      intt_butterfly(x[5], x[13], 32'd4401);
      intt_butterfly(x[6], x[14], 32'd4401);
      intt_butterfly(x[7], x[15], 32'd4401);
