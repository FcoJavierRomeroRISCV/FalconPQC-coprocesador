...
RTL_DIR = Path("hw/falcon_intt512_dma_accel/rtl")
IP_DIR = Path("hw/falcon_intt512_dma_accel")
APP_DIR = Path("sw/applications/falcon_intt512_dma_test")
...
if len(nums) < N:
    raise RuntimeError(f"iGMb tiene solo {len(nums)} valores, necesito al menos {N}")

iGMb = nums[:N]

print(f"iGMb detectada con {len(nums)} valores. Usando iGMb[0..{N - 1}].")
...
rtl = f"""module falcon_intt512_dma_accel (
    input logic clk_i,
    input logic rst_ni,
  ...
  localparam logic [31:0] NI512 = 32'd128;
  ...
  task automatic scale_final(inout logic [31:0] x [0:N-1]);
    begin
      for (int i = 0; i < N; i++) begin
        x[i] = montgomery_mul(x[i], NI512);
      end
    end
  endtask

  task automatic intt512_compute(inout logic [31:0] x [0:N-1]);
    int unsigned m;
    int unsigned hm;
    int unsigned t;
    int unsigned i;
...
(RTL_DIR / "falcon_intt512_dma_accel.sv").write_text(rtl)

(IP_DIR / "falcon_intt512_dma_accel.core").write_text("""CAPI=2:

name: javier:ip:falcon_intt512_dma_accel
description: Falcon iNTT512 DMA HW FIFO accelerator.

filesets:
  rtl:
    files:
    - rtl/falcon_intt512_dma_accel.sv
    - falcon_intt512_dma_accel.vlt : {file_type: vlt}
...
targets:
  default:
    filesets:
    - rtl
    toplevel: falcon_intt512_dma_accel
""")

(IP_DIR / "falcon_intt512_dma_accel.vlt").write_text("""`verilator_config

lint_off -rule SYNCASYNCNET -file "*falcon_intt512_dma_accel*"
lint_off -rule UNUSEDSIGNAL -file "*falcon_intt512_dma_accel*"
lint_off -rule WIDTHTRUNC -file "*falcon_intt512_dma_accel*"
""")
...
print("Generado:")
print(" - hw/falcon_intt512_dma_accel/rtl/falcon_intt512_dma_accel.sv")
print(" - hw/falcon_intt512_dma_accel/falcon_intt512_dma_accel.core")
print(" - hw/falcon_intt512_dma_accel/falcon_intt512_dma_accel.vlt")
print(" - sw/applications/falcon_intt512_dma_test/main.cpp")
