...
RTL_DIR = Path("hw/falcon_ntt256_dma_accel/rtl")
IP_DIR = Path("hw/falcon_ntt256_dma_accel")
APP_DIR = Path("sw/applications/falcon_ntt256_dma_test")
...
if not GMb_SOURCE.exists():
    raise FileNotFoundError(
        f"No existe {GMb_SOURCE}. Primero copia header.h con:\n"
        f"mkdir -p external_tables\n"
        f"cp RUTA_DEL_HEADER/header.h external_tables/header.h"
    )
...
if len(nums) < 256:
    raise RuntimeError(f"GMb tiene solo {len(nums)} valores, necesito al menos 256")

GMb = nums[:256]

print(f"GMb detectada con {len(nums)} valores. Usando GMb[0..255].")
...
rtl = f"""module falcon_ntt256_dma_accel (
    input logic clk_i,
    input logic rst_ni,
...
(RTL_DIR / "falcon_ntt256_dma_accel.sv").write_text(rtl)

(IP_DIR / "falcon_ntt256_dma_accel.core").write_text("""CAPI=2:

name: javier:ip:falcon_ntt256_dma_accel
description: Falcon NTT256 DMA HW FIFO accelerator.

filesets:
  rtl:
    files:
    - rtl/falcon_ntt256_dma_accel.sv
    - falcon_ntt256_dma_accel.vlt : {file_type: vlt}
...
targets:
  default:
    filesets:
    - rtl
    toplevel: falcon_ntt256_dma_accel
""")

(IP_DIR / "falcon_ntt256_dma_accel.vlt").write_text("""`verilator_config

lint_off -rule SYNCASYNCNET -file "*falcon_ntt256_dma_accel*"
lint_off -rule UNUSEDSIGNAL -file "*falcon_ntt256_dma_accel*"
lint_off -rule WIDTHTRUNC -file "*falcon_ntt256_dma_accel*"
""")
...
print("Generado:")
print(" - hw/falcon_ntt256_dma_accel/rtl/falcon_ntt256_dma_accel.sv")
print(" - hw/falcon_ntt256_dma_accel/falcon_ntt256_dma_accel.core")
print(" - hw/falcon_ntt256_dma_accel/falcon_ntt256_dma_accel.vlt")
print(" - sw/applications/falcon_ntt256_dma_test/main.c")
