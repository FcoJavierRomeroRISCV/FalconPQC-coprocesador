...
#define CH_NTT256   0u
#define CH_INTT256  1u

#define VEC_SIZE 256u
...
static const uint32_t GMb[VEC_SIZE] = {
    ...
    932u, 10229u, 8927u, 7642u, 3512u, 9298u, 237u, 5858u,
...
static int run_chain_test(uint32_t test_id) {
    ...
    printf("Running NTT256 -> iNTT256 chain test %u\n", test_id);

    print_first_last("Input  ", input_data);

    if (!run_dma_transfer(CH_NTT256, input_data, ntt_data, VEC_SIZE)) {
        printf("NTT256 DMA transfer failed\n");
        return 0;
    }

    print_first_last("NTT256 ", ntt_data);

    if (!run_dma_transfer(CH_INTT256, ntt_data, final_data, VEC_SIZE)) {
        printf("iNTT256 DMA transfer failed\n");
        return 0;
    }
    ...
    if (pass) {
        printf("NTT256 -> iNTT256 DMA chain Test PASS\n");
    } else {
        printf("NTT256 -> iNTT256 DMA chain Test FAIL\n");
    }
...
int main(void) {
    ...
    printf("Falcon NTT256 -> iNTT256 DMA chain test\n");
    ...
    if (pass) {
        printf("Falcon NTT256 -> iNTT256 DMA chain test PASS\n");
        return 0;
    } else {
        printf("Falcon NTT256 -> iNTT256 DMA chain test FAIL\n");
        return 1;
    }
}
