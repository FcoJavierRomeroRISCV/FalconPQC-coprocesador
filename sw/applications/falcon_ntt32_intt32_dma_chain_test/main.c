...
static int run_chain_test(const uint32_t test_input[VEC_SIZE]) {
    ...
    printf("Running NTT32 -> iNTT32 chain test\n");
    print_vector("Input:  ", input_data);

    if (!run_dma_transfer(CH_NTT32, input_data, ntt_data, VEC_SIZE)) {
        printf("NTT32 DMA transfer failed\n");
        return 0;
    }

    print_vector("NTT32:  ", ntt_data);

    if (!run_dma_transfer(CH_INTT32, ntt_data, final_data, VEC_SIZE)) {
        printf("iNTT32 DMA transfer failed\n");
        return 0;
    }
    ...
    if (pass) {
        printf("NTT32 -> iNTT32 DMA chain Test PASS\n");
    } else {
        printf("NTT32 -> iNTT32 DMA chain Test FAIL\n");
    }
...
int main(void) {
    ...
    printf("Falcon NTT32 -> iNTT32 DMA chain test\n");
    ...
    if (pass) {
        printf("Falcon NTT32 -> iNTT32 DMA chain test PASS\n");
        return 0;
    } else {
        printf("Falcon NTT32 -> iNTT32 DMA chain test FAIL\n");
        return 1;
    }
}
