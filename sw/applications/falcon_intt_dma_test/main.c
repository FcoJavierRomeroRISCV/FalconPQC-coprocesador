...
static void intt8_ref(uint32_t a[INTT_SIZE]) {
    intt_butterfly_ref(&a[0], &a[1], 2530u);
    intt_butterfly_ref(&a[2], &a[3], 6014u);
    intt_butterfly_ref(&a[4], &a[5], 7947u);
    intt_butterfly_ref(&a[6], &a[7], 5329u);

    intt_butterfly_ref(&a[0], &a[2], 1081u);
    intt_butterfly_ref(&a[1], &a[3], 1081u);
    intt_butterfly_ref(&a[4], &a[6], 1229u);
    intt_butterfly_ref(&a[5], &a[7], 1229u);

    intt_butterfly_ref(&a[0], &a[4], 4401u);
    intt_butterfly_ref(&a[1], &a[5], 4401u);
    intt_butterfly_ref(&a[2], &a[6], 4401u);
...
static int run_dma_intt_test(const uint32_t test_input[INTT_SIZE]) {
    int pass = 1;
    ...
    if (pass) {
        printf("iNTT8 DMA Test PASS\n");
    } else {
        printf("iNTT8 DMA Test FAIL\n");
    }
...
int main(void) {
    ...
    printf("Falcon iNTT8 DMA accelerator test\n");
    ...
    pass &= run_dma_intt_test(test0);
    pass &= run_dma_intt_test(test1);
    pass &= run_dma_intt_test(test2);
    pass &= run_dma_intt_test(test3);

    if (pass) {
        printf("Falcon iNTT8 DMA accelerator test PASS\n");
        return 0;
    } else {
        printf("Falcon iNTT8 DMA accelerator test FAIL\n");
        return 1;
    }
}
