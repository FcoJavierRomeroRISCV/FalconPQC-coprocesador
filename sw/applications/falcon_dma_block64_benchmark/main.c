...
static int run_block64_benchmark(
    const char *name,
    uint32_t *input,
    uint32_t *ntt,
    uint32_t *final,
    ...
    printf("==== %s NTT64/iNTT64 block benchmark ====\n", name);
    ...
    for (uint32_t b = 0; b < blocks; b++) {
        ...
        if (!run_dma_transfer(
            ...
            printf("NTT64 DMA failed at block %u\n", b);
            return 0;
        }
        ...
        if (!run_dma_transfer(
            ...
            printf("iNTT64 DMA failed at block %u\n", b);
            return 0;
        }
    }
    ...
    if (check_result(input, final, size)) {
        printf("%s NTT64/iNTT64 block benchmark PASS\n", name);
        return 1;
    } else {
        printf("%s NTT64/iNTT64 block benchmark FAIL\n", name);
        return 0;
    }
}
...
int main(void) {
    ...
    printf("Falcon DMA block64 benchmark test\n");
    ...
    if (pass) {
        printf("Falcon DMA block64 benchmark test PASS\n");
        return 0;
    } else {
        printf("Falcon DMA block64 benchmark test FAIL\n");
        return 1;
    }
}
